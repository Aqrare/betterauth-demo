...
**重要**:
- トークンには**ロール情報のみ**が含まれ、パーミッション一覧は含まれない
- `serviceId` は `organizationId` と同義（Better Auth の organization プラグインを services テーブルで管理）
...
```
1. Frontend → Resource MS: リクエスト + アクセストークン + X-Service-ID ヘッダー
2. Resource MS: トークン検証（JWTから serviceId を取得）
3. Resource MS: ロール → パーミッション変換（permissions.tsで管理）
4. Resource MS → OPA: {
     user: { serviceId, permissions },
     request: { serviceId, method, path }
   }
5. OPA: マルチテナンシーチェック（ヘッダーの serviceId == トークンの serviceId）
6. OPA: パーミッションチェック
7. OPA → Resource MS: { allow: true/false }
8. Resource MS: 判定結果に基づきレスポンス
```
